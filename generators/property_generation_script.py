...
with open("Properties.lua", "w") as lua_file:
    ...
    with open("generators/PropertyCards.csv") as property_cards:
        ...
        for property in reader:
            lua_file.write(f"        [\"{property['name']}\"] = Property.new(\"{property['name']}\", {property['price']}, \"{property['color']}\", {{{property['baserent']}, {property['onehouserent']}, {property['twohouserent']}, {property['threehouserent']}, {property['fourhouserent']}, {property['hotelrent']}, {property['skyscraperrent']}}}, {property['housecost']}),\n")
    
    lua_file.write("        -- Railroads\n")
    for railroad in ["Reading Railroad", "Pennsylvania Railroad", "B&O Railroad", "Short Line"]:
        lua_file.write(f"        [\"{railroad}\"] = Property.new(\"{railroad}\", 200, \"rail\", {{25, 50, 100, 200}}, 100),\n")
